...
#         "phi_dec_prefix": "".join(map(str, bits)),
#         "phi_dec": bits
#     }
...
def _npify_ciphertext_C(C: dict) -> dict:
    """
    Convert every C[k] into a numpy int array.
    Works for vectors and matrices (list or list-of-lists).
    """
    out = {}
    for k, v in C.items():
        out[k] = np.array(v, dtype=int)
    return out
...
def decrypt_with_userkey(ct_bundle: dict, sk_bundle: dict):
    ...
    if not usr.satisfiable:
        return {
            "ok": False,
            "satisfiable": False,
            "error": "User attributes do not satisfy policy."
        }
    ...
    for θ in range(N):
        # c_A_θ is a list-of-polys (matrix-ish), SK[attr+θ] matches poly_dotprod shape in your scheme
        e_share = poly_dotprod(C[f"c_A_{θ}"], SK[attr + θ])
    ...
    # ---- outputs
    msg_len = len(bits)
    prefix_len = min(128, len(bits))
    prefix = "".join(map(str, bits[:prefix_len]))
    ...
    return {
        ...
        # "msg_len": len(bits),
        "phi_dec": bits,
        "phi_dec_prefix": prefix,
        # "phi_dec_prefix_len": prefix_len,
    }